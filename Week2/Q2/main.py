...
def alpha_beta_pruning(history_obj, alpha, beta, max_player_flag):
    ...
    if history_obj.history not in visited_histories_list:
        visited_histories_list.append(history_obj.history)

        if max_player_flag is True:
            bestVal=-math.inf
            for action in history_obj.get_valid_actions():
                value=alpha_beta_pruning(history_obj.update_history(action),alpha,beta,False)
            ...
            bestVal=math.inf
            for action in history_obj.get_valid_actions():
                value=alpha_beta_pruning(history_obj.update_history(action),alpha,beta,True)
